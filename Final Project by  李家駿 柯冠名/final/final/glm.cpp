...
glmReadMTL(GLMmodel* model, char* name)
{
	...
	strcpy(dir, glmDirName(model->pathname));
	...
	strcpy(model->materials[0].name, "defalut");
	...
	while (fscanf(file, "%s", buf) != EOF) {
		...
		case 'n':               /* newmtl */
			...
			strcpy(model->materials[nummaterials].name, buf);
			break;
		case 'N':
...
glmReadOBJ(char* filename)
{
	...
	strcpy(model->pathname, filename);
